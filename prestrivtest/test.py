...
def message_chop(message):
    n = 8
    mlist = []
    for index in range(0, len(message), 8):
        i = message[index : index + n]
        if len(i) == 8:
            mlist.append(i)
        else:
            while len(i) != 8:
...
def present_print(pres, message):
    enc_message = []
    dec_message = []
    new_message = message_chop(message)
    start1 = time.time()
    for mitem in new_message:
        enc_message.append(pres.encrypt(mitem))
    end1 = time.time()
    start2 = time.time()
    for eitem in enc_message:
        dec_message.append(pres.decrypt(eitem).decode("utf-8"))
    end2 = time.time()
    
    #dec_m = "".join(dec_message)
    #print("dec message = original message: " + dec_m == message)
    #print("dec message: " + dec_m)
...
def E_and_dh(KEY, IV, message):
    ...
    print("Encryption1h time:" + str(end1 - start1))
    print("Decryption1h time:" + str(end2 - start2))
...
def main():
    ...
    print("Encryption and Decryption Speeds (binary(b) and hexadecimal(h)):")
    print("Trivium 32 bytes:")
    E_and_db(KEY, IV, message1)
    E_and_dh(KEY, IV, message1)
    
    print("Trivium 150 bytes:")
    E_and_db(KEY, IV, message2)
    E_and_dh(KEY, IV, message2)
    
    print("Trivium 600 bytes:")
    E_and_db(KEY, IV, message3)
    E_and_dh(KEY, IV, message3)
    
    
    key_p = codecs.decode("0123456789abcdef0123456789abcdef", 'hex')
    cipher = Present(key_p)
    print()
    print("Present 32 bytes:")
    present_print(cipher, message1)
    print("Present 150 bytes:")
    present_print(cipher, message2)
    print("Present 600 bytes:")
    present_print(cipher, message3)
